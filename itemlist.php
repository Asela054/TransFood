...
if (($handle = fopen($csvFile, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, $delimiter)) !== FALSE) {
        ...
        $materialName  = trim($data[2]);
        $prefVendor    = trim($data[3]);
        $unitId        = trim($data[4]);

        if ($itemCode == '') continue; // skip empty codes
        ...
        // ---------- CHECK CATEGORY & UNIT ----------
        if (!$categoryId) {
            echo "<tr><td>$itemCode</td><td>$materialName</td><td>$categoryId</td><td>$prefVendor</td><td>$unitId</td><td style='color:red;'>❌ Category ID missing</td></tr>";
            continue;
        }

        if (!$unitId) {
            echo "<tr><td>$itemCode</td><td>$materialName</td><td>$categoryId</td><td>$prefVendor</td><td>$unitId</td><td style='color:red;'>❌ Unit ID missing</td></tr>";
            continue;
        }
        ...
        // If material name is empty, insert empty string
        $insertMaterial->bind_param("ssiii", $materialName, $itemCode, $userId, $categoryId, $unitId);
        ...
        if ($insertMaterial->execute()) {
            ...
            echo "<tr><td>$itemCode</td><td>$materialName</td><td>$categoryId</td><td>$prefVendor</td><td>$unitId</td><td style='color:blue;'>✅ Inserted (Supplier ID: $supplierId)</td></tr>";
        } else {
            echo "<tr><td>$itemCode</td><td>$materialName</td><td>$categoryId</td><td>$prefVendor</td><td>$unitId</td><td style='color:red;'>❌ Insert Failed</td></tr>";
        }
    ...
    if ($rowCount === 0) {
        echo "<tr><td colspan='6' style='color:red;'>⚠️ No rows found in CSV file. Please check delimiter (tab/comma).</td></tr>";
    }
