...
$joinQuery = "FROM `tbl_material_info` AS `u` LEFT JOIN `tbl_material_suppliers` AS `ua` ON (`ua`.`idtbl_material_suppliers` = `u`.`tbl_material_suppliers_idtbl_material_suppliers`) LEFT JOIN `tbl_material_category` AS `ub` ON (`ua`.`idtbl_material_category` = `u`.`tbl_material_category_idtbl_material_category`) LEFT JOIN `tbl_supplier` AS `uc` ON (`uc`.`idtbl_supplier` = `ua`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_unit` AS `ud` ON (`ud`.`idtbl_unit` = `u`.`tbl_unit_idtbl_unit`)";

$joinQuery = "FROM `tbl_material_info` AS `u` 
LEFT JOIN `tbl_material_suppliers` AS `ua` ON (`ua`.`tbl_material_info_idtbl_material_info` = `u`.`idtbl_material_info`) 
LEFT JOIN `tbl_supplier` AS `uc` ON (`uc`.`idtbl_supplier` = `ua`.`tbl_supplier_idtbl_supplier`) 
LEFT JOIN `tbl_material_category` AS `ub` ON (`ub`.`idtbl_material_category` = `u`.`tbl_material_category_idtbl_material_category`) 
LEFT JOIN `tbl_unit` AS `ud` ON (`ud`.`idtbl_unit` = `u`.`tbl_unit_idtbl_unit`)";
