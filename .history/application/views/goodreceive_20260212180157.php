...
<script>
    ...
    function updateCurrencySymbol() {
        ...
        var totalElement = $('#divtotal');
        
        // Get current numeric value (remove any existing currency symbols and spaces)
        var currentValue = totalElement.text().replace(/[^0-9.]/g, '');
        if (currentValue === '') currentValue = '0.00';
        
        // Add currency symbol based on selection
        if (currencyValue === '1') {
            totalElement.text('Rs. ' + currentValue);
        } else if (currencyValue === '2') {
            totalElement.text('$ ' + currentValue);
        } else {
            totalElement.text(currentValue); // No currency selected
        }
    }
    
    // Listen for changes in the currency select dropdown
    $('#currencytype').on('change', updateCurrencySymbol);
