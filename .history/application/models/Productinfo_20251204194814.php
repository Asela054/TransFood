...
public function ProductProfileModal()
{
    ...
    // ✅ PRODUCT DATA
    $sqlproduct = "SELECT `idtbl_product`, `prodcutname`, `productcode`, `productimg`, `desc`, `weight`, `retailprice`, `wholesaleprice`, `nopckperctn`, `mastercartoon`
                   FROM `tbl_product`
                   WHERE `idtbl_product` = ? AND `status` = ?";
    $product = $this->db->query($sqlproduct, array($productid, 1))->row();

    if (!$product) {
        return '<div class="alert alert-warning">Product not found.</div>';
    }

    // ✅ MATERIALS / BOM for this product
    $sqlbom = "SELECT pb.qty, pb.wastage, mi.materialname, mi.idtbl_material_info
               FROM tbl_product_bom pb
               LEFT JOIN tbl_material_info mi ON mi.idtbl_material_info = pb.tbl_material_info_idtbl_material_info
               WHERE pb.tbl_product_idtbl_product = ? AND pb.status = ?
               ORDER BY pb.idtbl_product_bom ASC";
    $bom = $this->db->query($sqlbom, array($productid, 1))->result();

    $html = '';

    // ✅ HEADER: Product Image and Details
    $html .= '
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-md-4 border text-center p-2">
                <img src="'.base_url('images/ProductImg/'.$product->productimg).'" class="img-fluid mb-1" alt="Product Image">
            </div>
            <div class="col-md-8">
                <h5 class="font-weight-bold">ITEM NAME: '.$product->prodcutname.'</h5>
    ...
    // ✅ MATERIALS TABLE
    $html .= '
    <label class="badge badge-success mb-1">Materials Required for Product</label>
    ...
    foreach ($bom as $row) {
        // ✅ Get latest supplier for this material
        $sqlsup = "SELECT s.suppliername, ms.unitprice
                   FROM tbl_material_suppliers ms
                   LEFT JOIN tbl_supplier s ON s.idtbl_supplier = ms.tbl_supplier_idtbl_supplier
                   WHERE ms.tbl_material_info_idtbl_material_info = ? AND ms.status = ?
                   ORDER BY ms.idtbl_material_suppliers DESC
                   LIMIT 1"; // Only latest supplier
        $sup = $this->db->query($sqlsup, array($row->idtbl_material_info, 1))->row();

        $supplier_name = $sup->suppliername ?? '';
        $unit_price = isset($sup->unitprice) ? number_format($sup->unitprice, 2) : '';
    ...
    // ✅ FINISHED GOOD ITEM DESCRIPTION TABLE
    $html .= '
    ...
    <table class="table table-bordered table-sm">
        ...
        <tbody>
            <tr>
                ...
                <td>'.$product->nopckperctn.'</td>
                <td>'.$product->mastercartoon.'</td>
            </tr>
        </tbody>
    </table>
    </div>
