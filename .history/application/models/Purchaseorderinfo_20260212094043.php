...
public function Purchaseorderinsertupdate() {
    ...
    // --- Check foreign key values before insert/update ---
    if(empty($supplier) || empty($location) || empty($ordertype) || empty($companyid) || empty($branchid)){
        echo json_encode(['status'=>0,'message'=>'Foreign key values missing for parent record']);
    ...
    if($recordOption == 1): // INSERT
        ...
        $i = ($respond->row()->count > 0) ? $respond->row()->count + 1 : 1;
        ...
        // Check parent insert error
        $error = $this->db->error();
        if($error['code'] != 0){
            $this->db->trans_rollback();
            echo json_encode(['status'=>0, 'message'=>'Parent insert failed', 'error_code'=>$error['code'], 'error_message'=>$error['message']]);
            return;
        }
        ...
        if($respond->row()->confirmstatus > 0):
            echo json_encode([
                'status'=>0,
                'message'=>'Cannot Edit Confirmed Or Reject Order'
            ]);
            $this->db->trans_rollback();
            return;
        endif;
        ...
        if($error['code'] != 0){
            $this->db->trans_rollback();
            echo json_encode(['status'=>0, 'message'=>'Parent update failed', 'error_code'=>$error['code'], 'error_message'=>$error['message']]);
            return;
        }
    ...
    if(!empty($tableData)){
        foreach($tableData as $row){
            ...
            if($currencytype == 1){
                $finalUnit  = $unit_lkr;
                $finalDisc  = $disc_lkr;
                $finalTotal = $total_lkr;
            } else {
                $finalUnit  = $unit_usd;
                $finalDisc  = $disc_usd;
                $finalTotal = $total_usd;
            }
            ...
            if($error['code'] != 0){
                $this->db->trans_rollback();
                echo json_encode(['status'=>0, 'message'=>'Child insert failed', 'error_code'=>$error['code'], 'error_message'=>$error['message']]);
                return;
            }
        }
    }
    ...
    if ($this->db->trans_status() === TRUE) {
        ...
        echo json_encode([
            'status'=>1,
            'message'=>($recordOption==1?'Record Added Successfully':'Record Update Successfully')
        ]);
    } else {
        $this->db->trans_rollback();
        echo json_encode(['status'=>0, 'message'=>'Record Error']);
    }
