...
class Productinfo extends CI_Model{
    ...
    public function buildProductProfileHTML($productid){

    // ✅ PRODUCT
    $product = $this->db->where('idtbl_product',$productid)->get('tbl_product')->row();
    ...
    $this->db->join('tbl_material_info','tbl_material_info.idtbl_material_info = tbl_product_bom.tbl_material_info_idtbl_material_info');
    $this->db->where('tbl_product_bom.tbl_product_idtbl_product',$productid);
    $bom = $this->db->get()->result();
...
<div class="row mb-2">
    <div class="col-md-4 border text-center p-2">
        <img src="<?php echo base_url('uploads/'.$product->productimg); ?>" class="img-fluid mb-1">
    </div>
    <div class="col-md-8">
        <h5><b>ITEM NAME : <?php echo $product->prodcutname; ?></b></h5>
        <div>Code : <?php echo $product->productcode; ?></div>
    </div>
</div>

<hr>

✅ <b>Raw Materials Chart</b>
...
<?php foreach($bom as $b){ 
    ...
    $sup = $this->db
        ->select('tbl_supplier.supname, tbl_material_suppliers.unitprice')
        ->from('tbl_material_suppliers')
        ->join('tbl_supplier','tbl_supplier.idtbl_supplier = tbl_material_suppliers.tbl_supplier_idtbl_supplier')
        ->where('tbl_material_suppliers.tbl_material_info_idtbl_material_info',$b->idtbl_material_info)
        ->order_by('tbl_material_suppliers.idtbl_material_suppliers','DESC')
        ->limit(3)
        ->get()->result();
?>

<tr>
    <td><?php echo $b->materialname; ?></td>
    <td><?php foreach($sup as $s){ echo $s->supname."<br>"; } ?></td>
    <td><?php echo $b->qty; ?></td>
    <td><?php foreach($sup as $s){ echo number_format($s->unitprice,2)."<br>"; } ?></td>
</tr>
...
✅ <b>Finished Good Item Description</b>
...
<tr>
    <td><?php echo $product->prodcutname; ?></td>
    <td><?php echo $product->nopckperctn; ?></td>
    <td><?php echo $product->mastercartoon; ?></td>
</tr>
</table>
...
<tr>
    <td><?php echo $b->materialname; ?></td>
    <td><?php echo $b->qty; ?></td>
    <td><?php echo $b->wastage; ?></td>
</tr>
<?php } ?>
</tbody>
</table>

✅ <b>Previous Shipment Details</b>
<table class="table table-bordered table-sm">
<tr>
    <th>Item</th>
    <th>Inv No</th>
