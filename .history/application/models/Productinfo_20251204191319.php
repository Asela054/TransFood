...
public function ProductProfileModal()
{
    ...
    // ✅ PRODUCT DATA
    $sqlproduct = "SELECT `prodcutname`, `productcode`, `productimg`, `desc`, `weight`, `retailprice`, `wholesaleprice`
                   FROM `tbl_product`
                   WHERE `idtbl_product` = ? AND `status` = ?";
    $product = $this->db->query($sqlproduct, array($productid, 1))->row();

    // ✅ BOM / MATERIALS for this product
    ...
    <div class="container-fluid">
        ...
        <!-- Materials / BOM -->
        <label class="badge badge-success mb-1">Materials Required</label>
    ...
    foreach ($bom as $row) {
        ...
        foreach ($suppliers as $sup) {
            $supplier_list .= $sup->supname . ' / ' . number_format($sup->unitprice, 2) . '<br>';
        }
