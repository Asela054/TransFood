...
foreach($tableData as $rowtabledata){
    ...
    $unit         = str_replace(',', '', $rowtabledata['col_4']); // LKR
    $discountlkr  = str_replace(',', '', $rowtabledata['col_5']);

    $salepriceusd = str_replace(',', '', $rowtabledata['col_6']); // USD
    $discountusd  = str_replace(',', '', $rowtabledata['col_7']);

    $unitperctn   = $rowtabledata['col_10'];
    $ctn          = $rowtabledata['col_11'];
    $qty          = $rowtabledata['col_12'];

    $nettotal     = str_replace(',', '', $rowtabledata['col_13']); // LKR total
    $totalusd     = str_replace(',', '', $rowtabledata['col_14']); // USD total

    // Decide which currency to use
    if(!empty($unit) && $unit > 0){
        $finalUnitPrice   = $unit;
        $finalDiscount    = $discountlkr;
        $finalTotal       = $nettotal;
    } else {
        $finalUnitPrice   = $salepriceusd;
        $finalDiscount    = $discountusd;
        $finalTotal       = $totalusd;
    }
