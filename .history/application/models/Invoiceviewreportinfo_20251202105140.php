...
class Invoiceviewreportinfo extends CI_Model {
    public function printreport($x) {
        ...
        // Define numberToWords function first (not inside the class method scope issue)
        // But note: You cannot define a function inside another function like this in proper PHP
        // Let's restructure this
        
        // We'll remove the nested function definition and define it as a class method or separate helper
        // For now, let's fix syntax errors and structure
        ...
        // Call the numberToWords method correctly
    ...
    // Add the numberToWords function as a class method
    private function numberToWords($number) {
        ...
        if ($integerPart > 0) {
            $string .= $this->convertNumberToWords($integerPart, $dictionary);
        } else {
            $string .= 'zero';
        }

        if ($decimalPart > 0) {
            $string .= ' point ' . $this->convertNumberToWords($decimalPart, $dictionary);
        }

        return $string;
    }
    
    private function convertNumberToWords($number, $dictionary) {
        $string = '';
        ...
        if ($number < 20) {
            ...
            $string .= $number % 100 ? ' and ' . $this->convertNumberToWords($number % 100, $dictionary) : '';
        } elseif ($number < 1000000) {
            $string .= $this->convertNumberToWords(intval($number / 1000), $dictionary) . ' thousand';
            $string .= $number % 1000 ? ' ' . $this->convertNumberToWords($number % 1000, $dictionary) : '';
        } elseif ($number < 1000000000) {
            $string .= $this->convertNumberToWords(intval($number / 1000000), $dictionary) . ' million';
            $string .= $number % 1000000 ? ' ' . $this->convertNumberToWords($number % 1000000, $dictionary) : '';
        } else {
            $string .= $this->convertNumberToWords(intval($number / 1000000000), $dictionary) . ' billion';
            $string .= $number % 1000000000 ? ' ' . $this->convertNumberToWords($number % 1000000000, $dictionary) : '';
        }
