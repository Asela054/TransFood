...
class Goodreceiveinfo extends CI_Model{
    ...
    public function Getporder(){
        // return purchase orders that are confirmed but still have
        // outstanding quantity to be received.  A PO is considered open
        // if the sum of received quantities across all GRNs is less than
        // the sum of ordered quantities in the PO details.
        $sql = "SELECT p.idtbl_porder, p.po_no
                FROM tbl_porder p
                WHERE p.status=1
                  AND p.confirmstatus=1
                  /* total ordered qty */
                  AND (
                      SELECT IFNULL(SUM(pd.qty),0)
                      FROM tbl_porder_detail pd
                      WHERE pd.tbl_porder_idtbl_porder = p.idtbl_porder
                        AND pd.status = 1
                  )
                  > /* total received qty */
                  (
                      SELECT IFNULL(SUM(gd.qty),0)
                      FROM tbl_grn g
                      JOIN tbl_grndetail gd ON gd.tbl_grn_idtbl_grn = g.idtbl_grn
                        AND gd.status = 1
                      WHERE g.tbl_porder_idtbl_porder = p.idtbl_porder
                        AND g.status = 1
                  )";
    ...
    public function Getproductaccoporder(){
        ...
        // only return materials from the PO that still have outstanding
        // quantity (ordered qty minus already receipted qty > 0)
        $sql = "SELECT mi.idtbl_material_info,
                       mi.materialinfocode,
                       mi.materialname
                FROM tbl_porder_detail pd
                JOIN tbl_material_info mi
                  ON mi.idtbl_material_info = pd.tbl_material_info_idtbl_material_info
                WHERE mi.status = ?
                  AND pd.tbl_porder_idtbl_porder = ?
                  AND pd.status = 1
                  AND pd.qty > IFNULL(
                      (
                        SELECT SUM(gd.qty)
    ...
    public function Getproductinfoaccoproduct(){
        ...
        if ($porderID != "") {
            ...
            $respond = $this->db->query($sql, array($recordID, $porderID));
    ...
    public function Goodreceiveconfirm($x, $y){
        ...
        if($confirmstatus==1):
            ...
            // determine whether this PO is now fully received
            $totalOrdered = 0;
            $totalReceived = 0;

            $sqlOrd = "SELECT IFNULL(SUM(qty),0) as tot FROM tbl_porder_detail
                        WHERE status=1 AND tbl_porder_idtbl_porder=?";
            $resOrd = $this->db->query($sqlOrd, array($porderID));
            $totalOrdered = $resOrd->row(0)->tot;

            $sqlRecv = "SELECT IFNULL(SUM(gd.qty),0) as tot
                         FROM tbl_grn g
                         JOIN tbl_grndetail gd ON gd.tbl_grn_idtbl_grn = g.idtbl_grn
                           AND gd.status=1
                         WHERE g.tbl_porder_idtbl_porder=?
                           AND g.status=1";
            $resRecv = $this->db->query($sqlRecv, array($porderID));
            $totalReceived = $resRecv->row(0)->tot;
