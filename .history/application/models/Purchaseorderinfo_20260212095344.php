...
class Purchaseorderinfo extends CI_Model{
    ...
    public function Purchaseorderinsertupdate(){
        ...
        $currencytype=$this->input->post('currencytype'); // 1 = LKR, 2 = USD
        ...
        // Determine which total to use based on currency type (1 = LKR, 2 = USD)
        ...
        $finalTotal = !empty($finalTotal) ? $finalTotal : 0;
        $finalDiscount = !empty($finalDiscount) ? $finalDiscount : 0;
        $usdrate = !empty($usdrate) ? str_replace(',', '', $usdrate) : 0;
        ...
        if($recordOption==1): // insert
            ...
            foreach($tableData as $key => $rowtabledata){
                ...
                $finalUnitPrice = !empty($finalUnitPrice) ? $finalUnitPrice : 0;
                $finalDiscountAmount = !empty($finalDiscountAmount) ? $finalDiscountAmount : 0;
                $finalTotalDetail = !empty($finalTotalDetail) ? $finalTotalDetail : 0;
                $qty = !empty($qty) ? $qty : 0;
                $unitperctn = !empty($unitperctn) ? $unitperctn : 0;
                $ctn = !empty($ctn) ? $ctn : 0;
                ...
                if(!$this->db->insert('tbl_porder_detail', $dataone)) {
                    ...
                    throw new Exception('Detail insert failed: ' . $error['message']);
                }
            }
            ...
            if ($this->db->trans_status() === TRUE) {
                ...
                $actionObj->title='';
                $actionObj->message='Record Error: ' . ($db_error['message'] ?? 'Unknown error');
                $actionObj->url='';
                $actionObj->target='_blank';
