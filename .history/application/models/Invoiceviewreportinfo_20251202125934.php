...
class Invoiceviewreportinfo extends CI_Model{
    public function printreport($x){
        ...
        $sql = "SELECT `tbl_invoice_detail`.`idtbl_invoice_detail`, `tbl_invoice_detail`.`qty`, `tbl_invoice_detail`.`saleprice`, `tbl_invoice_detail`.`total`,`tbl_product`.`productcode`, `tbl_product`.`productname`, `tbl_product`.`weight`,`tbl_customer_porder_detail`.`unitprice`
	...
	foreach($respond->result() as $rowlist){
		if ($rowCount % $pageLimit == 0 && $rowCount > 0) {
			...
			<tr>
				...
				<th style="text-align: center; width:42%; border: thin 1px solid;" scope="col">Unit</th>
		...
		$tblinvoice .= '
			<tr>			
				...
				<td style="border: thin 1px solid;padding:5px;">'.$rowlist->productcode.'</td>
				<td style="text-align: center; border: thin 1px solid;padding:5px;">'.$rowlist->productname.'</td>
