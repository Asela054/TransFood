...
public function ProductProfileModal()
{
    ...
    foreach ($bom as $row) {
        ...
        $suppliers = $this->db->query($sqlsup, array($row->idtbl_material_info, 1))->result();

        // Combine supplier names and costs
        $supplier_list = '';
        $cost_list = '';
        foreach ($suppliers as $sup) {
            $supplier_list .= $sup->suppliername . '<br>';
            $cost_list .= number_format($sup->unitprice, 2) . '<br>';
        }
