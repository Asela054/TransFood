...
class Productinfo extends CI_Model{
    ...
    public function ProductProfileModal()
    {
        ...
        // ✅ PRODUCT DATA
        $sqlproduct = "SELECT `idtbl_product`, `prodcutname`, `productcode`, `productimg`, `desc`, `weight`, `retailprice`, `wholesaleprice`
                    FROM `tbl_product`
                    WHERE `idtbl_product` = ? AND `status` = ?";
        $product = $this->db->query($sqlproduct, array($productid, 1))->row();

        // ✅ MATERIALS / BOM for this product
        $sqlbom = "SELECT pb.qty, pb.wastage, mi.materialname, mi.materialinfocode, u.unitcode, sp.su[[liername
                FROM tbl_product_bom pb
                LEFT JOIN tbl_material_info mi ON mi.idtbl_material_info = pb.tbl_material_info_idtbl_material_info
                LEFT JOIN tbl_material_suppliers ms ON ms.idtbl_material_suppliers = ms.tbl_material_info_idtbl_material_info
                LEFT JOIN tbl_supplier sp ON sp.idtbl_supplier = ms.tbl_supplier_idtbl_supplier
                LEFT JOIN tbl_unit u ON u.idtbl_unit = mi.tbl_unit_idtbl_unit
                WHERE pb.tbl_product_idtbl_product = ? AND pb.status = ?";

                $sqlbom = "SELECT pb.qty, pb.wastage, mi.materialname, mi.materialinfocode, u.unitcode, sp.supname, ms.unitprice
           FROM tbl_product_bom pb
           LEFT JOIN tbl_material_info mi ON mi.idtbl_material_info = pb.tbl_material_info_idtbl_material_info
           LEFT JOIN tbl_material_suppliers ms ON ms.tbl_material_info_idtbl_material_info = mi.idtbl_material_info
           LEFT JOIN tbl_supplier sp ON sp.idtbl_supplier = ms.tbl_supplier_idtbl_supplier
           LEFT JOIN tbl_unit u ON u.idtbl_unit = mi.tbl_unit_idtbl_unit
           WHERE pb.tbl_product_idtbl_product = ? AND pb.status = ?";
        ...
        foreach ($bom as $row) {
            $html .= '
                <tr>
                    ...
                    <td>'.$row->qty.'</td>
                    <td>'.$row->unitcode.'</td>
                    <td class="text-center">'.$row->wastage.'</td>
                </tr>';
            $count++;
        }
